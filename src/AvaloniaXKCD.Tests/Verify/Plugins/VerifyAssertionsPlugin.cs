...
public static class VerifyAssertionsPlugin
{
    ...
    public static void Assert<T>(Action<T> assert) => SharedAsserts.Add(Wrap(assert));

    public static void Assert<T>(Func<T, Task> assert) => SharedAsyncAsserts.Add(Wrap(assert));
    ...
    public static void Initialize()
    {
        ...
        VerifierSettings.OnVerify(
            ...
            after: () =>
            {
                AssertionsRun.Value = false;
            }
        ); // Reset the flag after each verification
    ...
    static Func<object, Task> Wrap<T>(Func<T, Task> assert) =>
        async _ =>
        {
            if (_ is T t)
                await assert(t);
    ...
    static bool TryGetAsyncAsserts(
        IReadOnlyDictionary<string, object> context,
        [NotNullWhen(true)] out List<Func<object, Task>>? value
    )
    {
    ...
    where T : notnull
{
    ...
    public static implicit operator Task<VerifyResult>(AssertionVerificationTask<T> verificationTask) =>
        verificationTask.AsTask();
}
