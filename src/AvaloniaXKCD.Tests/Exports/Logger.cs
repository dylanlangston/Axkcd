...
{
    TUnit.Core.Logging.LogLevel ConvertLogLevel(LogLevel logLevel) => logLevel switch
    {
    ...
    {
        if (TestContext.Current == null) return false;
    ...
    public override void Log(LogLevel level, Exception err,
        [System.Runtime.CompilerServices.CallerFilePath] string? file = null) => Log(level, err.ToString());
}