...
var degreeOfParallelismOption = new Option<int>(
    name: "--dop",
    aliases: ["-d"])
{
...
var rootCommand = new RootCommand("XKCD Sync Tool")
{
    outputOption,
    baseUrlOption,
    degreeOfParallelismOption
};
...
public class ComicSyncStateMachine : IAsyncDisposable
{
    ...
    {
        public abstract Task<SyncState?> ExecuteAsync(ComicSyncStateMachine context, CancellationToken cancellationToken);
    }

    private class InitializeState : SyncState
    {
        public override async Task<SyncState?> ExecuteAsync(ComicSyncStateMachine context, CancellationToken cancellationToken)
        {
    ...
    private class ProcessComicsState : SyncState
    {
        ...
        public override async Task<SyncState?> ExecuteAsync(ComicSyncStateMachine context, CancellationToken cancellationToken)
        {
            ...
            await AnsiConsole.Progress()
                ...
                {
                    var verificationTask = ctx.AddTask("[green]Verifying and downloading comics[/]", new ProgressTaskSettings
                    {
                        MaxValue = _latestComicNumber
                    });

                    await Parallel.ForEachAsync(Enumerable.Range(1, _latestComicNumber), po, async (number, token) =>
                    {
        ...
        private async Task ProcessSingleComic(ComicSyncStateMachine context, int number, CancellationToken cancellationToken)
        {
            ...
            if (File.Exists(jsonPath))
            {
                ...
                try
                {
                    comicToSave = (XKCDComic?)await JsonSerializer.DeserializeAsync(fileStream, typeof(XKCDComic), XKCDComicJsonSerializerContext.Default, cancellationToken);
                }
                catch (JsonException)
                {
            ...
            else
            {
                ...
                {
                    comicToSave = comicToSave with { Img = Path.Combine(comicToSave.Num.ToString(), Path.GetFileName(imgUri.LocalPath)) };
                    needsSave = true;
                }
                else if (Path.Combine(comicToSave.Num.ToString(), Path.GetFileName(comicToSave.Img)) is string expectedImg && comicToSave.Img != expectedImg)
                {
            ...
            if (needsSave)
            {
                await File.WriteAllTextAsync(jsonPath, JsonSerializer.Serialize(comicToSave, typeof(XKCDComic), XKCDComicJsonSerializerContext.Default), cancellationToken);
            }
            ...
            if (needsImgDownload)
            {
                ...
                var downloadSuccessful = await DownloadImageAsync(httpClient, new Uri(remoteComic!.Img), comicDirectory, comicToSave.Num, cancellationToken: cancellationToken);
                if (downloadSuccessful) await DownloadImageAsync(httpClient, new Uri(remoteComic!.Img2x), comicDirectory, comicToSave.Num, is2x: true, cancellationToken: cancellationToken);
            }
        }

        async Task<bool> DownloadImageAsync(HttpClient client, Uri url, DirectoryInfo targetDir, int comic, bool is2x = false, CancellationToken cancellationToken = default)
        {
            if (string.IsNullOrEmpty(url.LocalPath) || url.LocalPath == "/") return false;
            ...
            {
                AnsiConsole.MarkupLine($"[yellow]Cancellation token received. Halting download comic #{comic} for image {url}[/]");
                throw;
            }
            catch (Exception ex)
            {
                AnsiConsole.MarkupLine($"[yellow]Could not download comic #{comic} image {(is2x ? "(2x) " : "")}{url}: {ex.Message}[/]");
                return false;
            }
        }
    }
    ...
    private class CopyLatestJsonState : SyncState
    {
        ...
        public override Task<SyncState?> ExecuteAsync(ComicSyncStateMachine context, CancellationToken cancellationToken)
        {
            AnsiConsole.MarkupLine($"Copying latest comic info ([yellow]#{_latestComicNumber}[/]) to root output directory...");
            var sourcePath = Path.Combine(context._outputDirectory.FullName, _latestComicNumber.ToString(), "info.0.json");
            ...
            {
                AnsiConsole.MarkupLine($"[red]Could not find the source JSON file for the latest comic: {sourcePath}[/]");
            }
