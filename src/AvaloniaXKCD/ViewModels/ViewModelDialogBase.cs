...
public abstract class ViewModelDialogBase<T, T2, T3> : ViewModelDialogBase, IViewModelDialogBase<T, T2, T3>
    ...
    where T3 : ViewModelDialogBase<T, T2, T3>.ViewModelDialogBaseEventArgs
{
    ...
    {
        protected ViewModelDialogBaseEventArgs(T2 type, T self)
            : base(type, self) { }
    }
